...
class AuthLib {
  ...
  constructor(opts) {
    ...
    this.opts.maxRefreshTokens = this.opts.tokenDuration || 5;
  ...
  getAccessToken(user, type) {
    var jwtOpts = {expiresInSeconds: this.opts.tokenDuration, algorithm: 'HS256'};
  ...
  _validateAccessToken(req, token, done) {
    console.log('_validateAccessToken called', token, this.opts.secret);
    var obj;
    try {
